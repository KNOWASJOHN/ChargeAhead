(function () {
  ...
  function showDetail(s) {
    ...
    const existingBooking = bookings.get(s.id);
    const bookingAction = existingBooking
      ? `<button class="booking-secondary-btn" type="button" data-booking-details>Booking details</button>`
      : `<button class="booking-primary-btn" type="button" data-book-charger>Book charger</button>`;
    ...
    el.detailContent.innerHTML = `
      ...
      <section class="booking-card">
        <div class="booking-card-header">
          <div>
            <div class="booking-title">Reserve a charger</div>
            <div class="booking-subtitle">Demo booking and payment flow</div>
          </div>
        </div>
        <div class="booking-actions">${bookingAction}</div>
        <div class="booking-form-wrap hidden" data-booking-form-wrap>
          <form class="booking-form" data-booking-form>
            <label>Driver name<input name="driverName" type="text" placeholder="Your name" required /></label>
            <label>Start time<input name="startTime" type="datetime-local" required /></label>
            <label>Duration
              <select name="duration">
                <option value="30">30 minutes - INR 49</option>
                <option value="60">60 minutes - INR 79</option>
                <option value="90">90 minutes - INR 109</option>
              </select>
            </label>
            ...
            <button class="booking-primary-btn" type="submit">Continue to payment</button>
          </form>
        </div>
        <div class="payment-panel hidden" data-payment-panel>
          <div class="payment-label">Payment gateway</div>
          <div class="payment-summary" data-payment-summary></div>
          <button class="booking-primary-btn" type="button" data-pay-now>Pay now</button>
        </div>
        <div class="booking-confirmation hidden" data-booking-confirmation></div>
      </section>
    `;
    el.detailDrawer.classList.remove("hidden");

    const bookButton = el.detailContent.querySelector("[data-book-charger]");
    const detailsButton = el.detailContent.querySelector("[data-booking-details]");
    if (bookButton) {
      bookButton.addEventListener("click", () => {
        el.detailContent.querySelector("[data-booking-form-wrap]").classList.remove("hidden");
        bookButton.classList.add("hidden");
      });
    }
    if (detailsButton) {
      detailsButton.addEventListener("click", () => showBookingConfirmation(s, existingBooking));
    }

    const bookingForm = el.detailContent.querySelector("[data-booking-form]");
    if (bookingForm) {
      const startInput = bookingForm.elements.startTime;
      startInput.value = new Date(Date.now() + 30 * 60000).toISOString().slice(0, 16);
      bookingForm.addEventListener("submit", (event) => {
        event.preventDefault();
        const formData = new FormData(bookingForm);
        const duration = Number(formData.get("duration"));
        const amount = duration === 30 ? 49 : duration === 60 ? 79 : 109;
        ...
        bookingForm.closest("[data-booking-form-wrap]").classList.add("hidden");
        const payment = el.detailContent.querySelector("[data-payment-panel]");
        payment.classList.remove("hidden");
        payment.querySelector("[data-payment-summary]").textContent =
          `${pendingBooking.paymentMethod} - INR ${pendingBooking.amount} for ${pendingBooking.duration} minutes`;
        payment.querySelector("[data-pay-now]").onclick = () => {
          const booking = {
            ...pendingBooking,
            id: `CA-${Date.now().toString(36).toUpperCase()}`,
            stationName: s.name,
            stationAddress: s.address || "Address not provided",
          };
          bookings.set(s.id, booking);
          showBookingConfirmation(s, booking);
        };
      });
    }
  }

  function showBookingConfirmation(station, booking) {
    const confirmation = el.detailContent.querySelector("[data-booking-confirmation]");
    if (!confirmation) return;
    el.detailContent.querySelector("[data-booking-form-wrap]")?.classList.add("hidden");
    el.detailContent.querySelector("[data-payment-panel]")?.classList.add("hidden");
    el.detailContent.querySelector(".booking-actions")?.classList.add("hidden");
    confirmation.classList.remove("hidden");
    const date = new Date(booking.startTime).toLocaleString(undefined, {
      dateStyle: "medium",
      timeStyle: "short",
    });
    confirmation.innerHTML = `
      ...
      <div class="booking-detail-grid">
        <span>Reference</span><strong>${escapeHtml(booking.id)}</strong>
        <span>Driver</span><strong>${escapeHtml(booking.driverName)}</strong>
        <span>Start</span><strong>${escapeHtml(date)}</strong>
        <span>Duration</span><strong>${booking.duration} minutes</strong>
        <span>Amount</span><strong>INR ${booking.amount}</strong>
      </div>
      <div class="booking-qr" data-booking-qr aria-label="Booking QR code"></div>
      <div class="booking-qr-help">Show this QR code at the station.</div>
    `;
    const canvas = confirmation.querySelector("[data-booking-qr]");
    ...
    if (typeof window.QRCode === "function") {
      ...
      canvas.replaceWith("QR generator unavailable");
    }
  }
